-- Update the get_celebrities_with_subscription function to only return verified celebrities
...
BEGIN
  ...
  WHERE au.email NOT IN (SELECT admin_users.email FROM admin_users)
    AND cp.is_verified = true  -- Only show verified celebrities
    AND cp.is_available = true  -- Only show available celebrities
